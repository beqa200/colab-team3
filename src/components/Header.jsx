...
<header className={`p-4 bg-white ${isOpen ? 'flex flex-col items-center' : 'flex items-center justify-between'}`}> 
      ...
      <div className={`mt-4 flex flex-col items-center space-y-2 lg:flex-row lg:space-y-0 lg:space-x-4 ${isOpen ? 'block' : 'hidden lg:flex'}`}>
       ...
       <Link to={"/services"} className='text-[#a2724e]'>services</Link>
       <Link to={"/login"} className='text-[#a2724e]'>sign up</Link>
       <button className='text-[#a2724e]' onClick={()=> navigate('/registration') } >sign in  </button>
       
       <Link className='text-[#a2724e]' to={"/contact"}>contact</Link>
       <Link className='text-[#a2724e]'>start planning</Link>
      </div>
